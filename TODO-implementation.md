...
### 1.1 서버 컴포넌트 구현
- [ ] `getServerSideProps` 대신 서버 컴포넌트 패턴 적용
  ```typescript
  // 예시 코드
...
### 1.2 포스트 목록 페이지 구현
- [ ] 포스트 목록 페이지 (`app/blog/page.tsx`) 수정
  - [ ] URL 쿼리 파라미터에서 페이지와 카테고리 값 추출
  - [ ] API 호출하여 포스트 목록 데이터 가져오기
  - [ ] 로딩 상태 UI 구현
  - [ ] 오류 처리 UI 구현
  - [ ] 포스트 목록 렌더링
    - [ ] 각 포스트 카드 컴포넌트 구현
    - [ ] 카드에 제목, 요약, 카테고리, 날짜 표시

### 1.3 페이지네이션 구현
- [ ] 페이지네이션 컴포넌트 (`app/components/Pagination.tsx`) 생성
  - [ ] 현재 페이지, 전체 페이지, 페이지 이동 함수 props 설정
  - [ ] 페이지 번호 표시 및 이전/다음 버튼 구현
  - [ ] 활성 페이지 스타일링

### 1.4 카테고리 필터링 구현
- [ ] 사이드바 카테고리 컴포넌트 (`app/components/CategorySidebar.tsx`) 생성
  - [ ] 서버에서 모든 카테고리 데이터 가져오기
  - [ ] 카테고리 목록 렌더링
  - [ ] 현재 선택된 카테고리 하이라이트 처리
  - [ ] 카테고리 클릭 시 해당 카테고리로 필터링된 URL로 이동
  - [ ] 각 카테고리별 게시물 수 표시
  - [ ] 모바일 반응형 처리
...
### 2.1 서버 컴포넌트 구현
- [ ] 동적 경로 매개변수에서 `postId` 추출하여 API 호출

### 2.2 게시물 상세 페이지 구현
- [ ] 게시물 상세 페이지 (`app/posts/[postId]/page.tsx`) 생성
  - [ ] API에서 게시물 상세 정보 가져오기
  - [ ] 게시물 제목, 내용, 카테고리, 날짜 표시
  - [ ] Markdown 또는 HTML 렌더링 처리
  - [ ] 이전/다음 게시물 링크 (선택 사항)

### 2.3 댓글 섹션 구현
- [ ] 댓글 컴포넌트 (`app/components/Comments.tsx`) 생성
  - [ ] 클라이언트 컴포넌트로 설정 (`"use client"`)
  - [ ] `useEffect`를 사용해 컴포넌트 마운트 시 댓글 목록 가져오기
  - [ ] 댓글 목록 렌더링
  - [ ] 댓글 작성 폼 구현
    - [ ] 이름, 내용 입력 필드
    - [ ] 유효성 검사
...
### 4.1 로딩 및 에러 상태 컴포넌트
- [ ] 로딩 컴포넌트 (`app/components/Loading.tsx`) 생성
- [ ] 에러 메시지 컴포넌트 (`app/components/ErrorMessage.tsx`) 생성
