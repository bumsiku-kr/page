 ...
 */
export function usePostsQuery(page: number = 0, size: number = 10, sort: SortOption = 'createdAt,desc') {
...
export function usePostsByTagQuery(
  ...
  return useSWR(tag ? ['posts', 'tag', tag, page, size, sort] : null, () => api.posts.getList(page, size, tag, sort));
}
